...
import uk.gov.nationalarchives.tdr.transfer.service.api.controllers.{LoadController, ProcessingController}
...
object TransferServiceServer extends IOApp {
  ...
  private val processingController = ProcessingController()

  private val documentationEndpoints =
    SwaggerInterpreter().fromEndpoints[IO](loadController.endpoints ++ processingController.endpoints, openApiInfo)
  ...
  private val allRoutes =
    Http4sServerInterpreter[IO]().toRoutes(documentationEndpoints) <+> loadController.routes <+> processingController.routes <+> healthCheckRoute
