...
import uk.gov.nationalarchives.tdr.transfer.service.services.dataload.{DataLoadInitiation, DataLoadProcessor, DataLoadResultsHandler}

import java.util.UUID

class LoadController(dataLoadInitiation: DataLoadInitiation, dataLoadProcessor: DataLoadProcessor, dataLoadResultsHandler: DataLoadResultsHandler) extends BaseController {

  def endpoints: List[Endpoint[String, _ >: Unit with UUID, BackendException.AuthenticationError, _ >: LoadDetails with String <: Serializable, Any]] = List(
    initiateLoadEndpoint.endpoint,
    completeLoadEndpoint.endpoint,
    processLoadResultEndpoint.endpoint
  )
  ...
  val initiateLoadRoute: HttpRoutes[IO] =
    Http4sServerInterpreter[IO]().toRoutes(initiateLoadEndpoint.serverLogicSuccess(ac => _ => dataLoadInitiation.initiateLoad(ac.token)))
...
object LoadController {
  def apply() = new LoadController(DataLoadInitiation(), DataLoadProcessor(), DataLoadResultsHandler())
}
