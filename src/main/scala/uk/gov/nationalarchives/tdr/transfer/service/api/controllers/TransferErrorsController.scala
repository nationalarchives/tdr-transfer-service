...
class TransferErrorsController(transferErrors: TransferErrors)(implicit logger: SelfAwareStructuredLogger[IO]) extends BaseController {
  ...
  def endpoints: List[Endpoint[String, UUID, BackendException.AuthenticationError, List[Json], Any]] =
    List(getErrorsEndpoint.endpoint)

  private val getErrorsEndpoint: PartialServerEndpoint[String, AuthenticatedContext, UUID, BackendException.AuthenticationError, List[Json], Any, IO] =
