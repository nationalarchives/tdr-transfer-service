...
import uk.gov.nationalarchives.tdr.transfer.service.api.model.Common.TransferFunction.Load
...
class TransferErrorsController(transferErrors: TransferErrors)(implicit logger: SelfAwareStructuredLogger[IO]) extends BaseController {
  private val transferFunction: EndpointInput[String] = path(Load.toString)

  override def routes: HttpRoutes[IO] = getErrorsRoute

  def endpoints: List[Endpoint[String, (String, UUID), BackendException.AuthenticationError, TransferErrorsResults, Any]] =
    List(getErrorsEndpoint.endpoint)

  private val getErrorsEndpoint: PartialServerEndpoint[String, AuthenticatedContext, (String, UUID), BackendException.AuthenticationError, TransferErrorsResults, Any, IO] =
    securedWithStandardUserBearer
      ...
      .in("errors" / transferFunction / transferId)
